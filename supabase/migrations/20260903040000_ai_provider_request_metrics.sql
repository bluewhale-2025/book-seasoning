...
add column request_metrics jsonb
check (request_metrics is null or jsonb_typeof(request_metrics) = 'object');

comment on column private.ai_provider_runs.request_metrics is
  'Content-free request byte sizes and output budget used to diagnose provider latency.';
...
begin
  if p_attempt_no <= 0
    ...
    or p_latency_ms < 0
    or p_request_metrics is null
    or jsonb_typeof(p_request_metrics) <> 'object' then
  ...
  run_payload := case when p_status = 'SUCCEEDED' then
    pg_catalog.jsonb_build_object(
      ...
      'requestMetrics', p_request_metrics,
      'usage', p_usage
    )
    else null end;
  ...
  if found then
    if existing_run.prompt_version is distinct from p_prompt_version
      ...
      or existing_run.error_code is distinct from p_error_code
      or existing_run.request_metrics is distinct from p_request_metrics then
  ...
  insert into private.ai_provider_runs (
    ...
    usage, error_code, request_metrics, canonical_run
  ) values (
    p_job_id, p_attempt_no, p_task_alias, p_prompt_version,
    p_output_schema_version, p_provider, p_model, p_reasoning_effort,
    p_status, p_response_id, p_latency_ms, p_usage, p_error_code,
    p_request_metrics, run_payload
...
comment on function private.record_ai_provider_run(
  uuid, integer, text, text, text, text, text, text, text,
  text, integer, jsonb, text, jsonb
) is 'Records content-free provider diagnostics including request size metadata.';
