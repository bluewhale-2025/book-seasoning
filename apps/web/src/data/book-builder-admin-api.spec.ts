...
describe("HttpBookBuilderAdminApi", () => {
  it("creates a Pack with the public admin contract", async () => {
    const fetcher = vi.fn<typeof fetch>().mockResolvedValue(new Response(JSON.stringify({ packVersionId, status: "DRAFT", revision: 0, run, duplicate: false, serverTime }), { status: 201 }));
    const api = createApi(fetcher);

    await expect(api.createPack({ commandId: "a1000000-0000-4000-8000-000000000003", title: "  데미안  ", author: "헤르만 헤세" })).resolves.toMatchObject({ packVersionId, status: "DRAFT" });
    expect(fetcher).toHaveBeenCalledWith("http://api.test/v1/admin/book-context/packs", expect.objectContaining({
      method: "POST",
      body: JSON.stringify({ commandId: "a1000000-0000-4000-8000-000000000003", title: "데미안", author: "헤르만 헤세" }),
    }));
  });
  ...
  it("uses the current revision for lifecycle commands", async () => {
    ...
    const api = createApi(fetcher);
    await api.requestReview(packVersionId, { commandId: "a1000000-0000-4000-8000-000000000004", expectedRevision: 3 });
