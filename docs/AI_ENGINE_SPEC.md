...
각 operation은 `baseVersion`, stable item id와 evidence refs를 가진다. validator는 schema, 길이, reference 존재성, session/Pack 소속, cursor 범위, relation target, private variant 부재와 금지 표현을 확인한다. 모델이 선택한 공개 message seq 또는 Book item id는 서버가 같은 context의 opaque UUID로 정규화하고, 존재하지 않는 Wiki entity를 가리키는 relation·coverage edge는 의미 없는 dangling link로 제거한다. 알 수 없는 evidence locator나 그 밖의 validation 실패는 부분 적용하지 않고 patch 전체를 거절한다.
...
- Policy의 `WAIT`, 단일 action, metric 조합과 objective silence 회귀
- AI 개입 뒤 참가자 응답 전에는 시간 기반 평가를 다시 enqueue하지 않음
- provider schema/request byte, output budget, cached/input/output token과 latency 진단
...
6. **S5-6 — Evaluator (완료, 2026-09-02):** provider-neutral `PublicEvaluator` port와 `PublicEvaluationService`가 고정 job cursor로 PUBLIC Context를 만들고 Evaluator output을 검증한 뒤 S5-5 Wiki/evaluation commit으로 전달한다. output validator는 `OPENING | CORE | EXTENDED | SYNTHESIS | ENDED(Final Wiki)` phase, job/session/base/cursor/exact Pack, 지표·level별 allow-list reason code, context에 실제 제공된 PUBLIC reference, reference materialization, participant 소속과 Book Grounding의 공개 토론 연결을 fail-closed로 확인한다. S5-9 live 검증에서 top-level 평가와 typed patch에 중복되는 metrics/topic/관점/책 근거/참여 projection은 검증된 top-level 값을 canonical source로 삼는다. 2026-09-03 `public-evaluator.v4`에는 provider 입력에 exact allowed evidence catalog와 required envelope를 추가하고, metric별 level/reason-code 조합을 Provider JSON Schema에 직접 고정했다. 공개 message seq·Book item id로 선택된 opaque reference는 같은 context의 실제 UUID로 복원하고, 중복 reference와 존재하지 않는 Wiki entity를 향한 dangling relation·coverage edge만 제거한다. v7부터 일반 incremental Provider는 `wikiPatch`를 작성하지 않고 검증된 top-level 관찰만 반환하며, 서버가 그 관찰을 typed Wiki operation으로 결정적으로 투영한다. 의미적 삭제·issue·coverage 정리가 필요한 Topic Checkpoint와 Final만 Provider patch를 유지한다. 알 수 없는 evidence locator와 나머지 최종 Wiki 모순은 DB 접근 전에 거절한다. fake Evaluator로 incremental lifecycle과 stale suppression을 검증했고, worker routing은 invalid output을 retry 대상으로 분류하되 Closing을 suppress한다. S5-8의 `GatewayPublicEvaluator`가 이 port를 실제 provider adapter에 연결하며, worker key가 없으면 결과 생성을 가장하지 않고 `EVALUATOR_PROVIDER_NOT_CONFIGURED`로 suppress한다. `suggestedAction`은 canonical evaluation에 남는 참고 신호일 뿐 Policy action을 만들지 않는다.
...
9. **S5-9 — orchestration과 강화 (완료, 2026-09-02):** session 시작 transaction은 Opening을, participant message trigger와 Cron reconciler는 message batch·고유 발언자·침묵·의제 지속 candidate를 durable `PUBLIC_EVALUATION` job으로 enqueue한다. trigger/reason은 content-free private directive에 명시해 worker가 추론하지 않으며, active job과 unique orchestration key로 중복을 억제한다. stale base·Policy·연장 결과는 최신 Wiki/cursor로 최대 3회 bounded refresh하고 이전 attempt는 공개 효과를 만들지 않는다. 종료 7분 전 window는 비차단 `PENDING | READY | UNAVAILABLE` 연장 의견을 만들고 방장 판단 권한을 바꾸지 않는다. `POST /v1/rooms/:roomId/session/ai-help`는 방장·phase/version·idempotency·one-in-flight·90초 재요청 실험값을 검증하고 네 가지 고정 reason만 받아 비동기 상태를 반환한다. 처리 상태는 방장 snapshot에만 보이며 성공한 non-WAIT Host 메시지만 모든 참가자에게 한 번 공개된다. 60초 queue lease를 20초마다 DB와 PGMQ에 함께 연장해 최대 60초 provider 호출 두 개를 포함하는 긴 job의 attempt fencing을 유지한다. provider timeout/connection은 content-free code로 분류하고 SDK 재시도는 0으로 둬 Queue가 bounded retry를 단독 소유한다. degraded pgTAP은 자동 실패가 대화를 막지 않고 host-help `FAILED`, 연장 `UNAVAILABLE`만 안전하게 노출함을 검증했다. 실제 Luna Evaluator v2 → Wiki → `HOST_HELP/DEEPEN` Policy → Terra Host → DB 흐름도 transient provider 실패 뒤 두 번째 attempt에 성공했고, AI Host 메시지·intervention은 각각 한 건, 일반 참가자의 host request state는 `null`, 합성 `AI_PRIVATE` canary는 비노출이었다.

2026-09-03 안정화에서 `public-evaluator.v5`는 top-level projection을 patch에 반복 요구하지 않고, 반복 provider schema를 `$ref` definition으로 공유해 직렬화 크기를 30KB 이하로 제한했다. Evaluator Pack subset은 최대 24개, output budget은 Incremental 6,000·Checkpoint 8,000·Final 12,000 token으로 분리한다. provider 진단에는 원문 없이 input/instructions/schema/total byte, output budget과 cached input token을 추가했다. 실제 `SILENCE` trigger와 `silenceSeconds >= 60`은 지연된 Evaluator의 Activity 값보다 우선하며, 시간 기반 trigger는 마지막 AI 개입 뒤 참가자가 새로 응답했고 90초 cooldown도 끝난 경우에만 허용한다. 새 human message 기반 평가와 Wiki 반영은 이 gate로 막지 않는다. `AI_EVALUATOR_REPEAT_COUNT=1..5` live eval은 프로덕션과 같은 Wiki canonicalization·commit 검증 경로를 사용한다.

같은 날 추가 안정화에서 `public-evaluator.v6`는 provider가 UUID-bearing evidence 객체를 반복 출력하지 않고 입력의 `allowedEvidenceCatalog`에 있는 0-based index만 반환하게 한다. 서버는 index를 exact PUBLIC reference로 복원한 뒤 기존 canonical schema·allow-list·materialization·privacy 검증을 그대로 다시 적용하며, 범위 밖 index와 Book item 자리에 message index를 쓴 출력은 fail-closed로 거절한다. 새 공개 메시지가 없는 `SILENCE` job은 현재 Wiki가 같은 cursor를 이미 덮고 있고 같은 cursor의 committed evaluation이 있을 때만 그 canonical evaluation을 재검증해 사용한다. 이 경로에서도 현재 job의 immutable evaluation과 Policy action은 새로 확정하지만 Evaluator provider 호출과 의미 없는 Wiki 재작성은 생략한다. authoritative message cursor, Wiki version/cursor, active attempt 중 하나라도 달라지면 재사용하지 않고 정상 Evaluator 경로로 돌아간다. 시간 reconciler는 advisory lock과 최대 100개 active session scan을 유지한 채 1초 주기로 실행되어 60초 threshold를 1초 해상도로 감지한다.

`public-evaluator.v7`은 incremental 출력에서 `wikiPatch`를 제거해 빠른 관찰과 Wiki operation 작성을 분리한다. message batch·participation threshold는 충분한 새 PUBLIC 발언이 있을 때만 실행하고, 무변화 silence는 v6의 same-cursor 평가를 재사용하며, Topic Checkpoint·Final만 의미적 patch를 생성한다. Pack subset은 문서 앞부분이 아니라 현재 topic, 최근 6개 발언, 기존 Wiki 관점·Book Grounding과 section 우선순위로 점수화해 최대 8개를 선택하고 기존 grounding item을 우선 보존한다. Opening도 PUBLIC prep 기준 최대 8개를 사용한다. Opening `TRANSITION`은 서로 다른 요약 수만 세지 않고, 서로 다른 참가자의 서로 다른 PUBLIC message가 각 관점 근거로 연결된 경우에만 허용한다. 같은 근거를 복제한 관점과 한 참가자 근거만 있는 관점은 `WAIT`다.
